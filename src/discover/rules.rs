...
pub const RULES: &[RtkRule] = &[
    ...
    RtkRule {
        pattern: r"^bun\s+(install|add|remove|build|init|create|upgrade|pm|link|unlink|dev|repl|completions)\b",
        rtk_cmd: "rtk bun",
    ...
    RtkRule {
        pattern: r"^bun\s+(run|x)(\s|$)",
        rtk_cmd: "rtk bun run",
        rewrite_prefixes: &["bun run", "bun x"],
        category: "PackageManager",
        savings_pct: 70.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?tsc(\s|$)",
        rtk_cmd: "rtk tsc",
        rewrite_prefixes: &[
            "bun run tsc",
            "bun tsc",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?(biome|eslint|lint)(\s|$)",
        rtk_cmd: "rtk lint",
        rewrite_prefixes: &[
            "biome",
            "bun biome",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?prettier",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?next\s+build",
        rtk_cmd: "rtk next",
        rewrite_prefixes: &[
            "bun next build",
            "bun run next build",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?jest(\s+run)?(\s|$)",
        rtk_cmd: "rtk jest",
        rewrite_prefixes: &[
            "bun jest run",
            "bun jest",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?vitest(\s+run)?(\s|$)",
        rtk_cmd: "rtk vitest",
        rewrite_prefixes: &[
            "bun run vitest run",
            "bun run vitest",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?playwright",
    ...
    RtkRule {
        pattern: r"^((p?np(m|x)|p?npm\s+(exec|run|run-script)|npm\s+(rum|urn|x)|pnpm\s+dlx|bunx?|bun\s+(x|run))\s+)?prisma",
        rtk_cmd: "rtk prisma",
        rewrite_prefixes: &[
            "bun prisma",
            "bun run prisma",
    ...
    RtkRule {
        pattern: r"^go\s+(test|build|vet|fmt|generate|run|install|mod|get|clean|env|doc|version|tool|work|list)",
        rtk_cmd: "rtk go",
        rewrite_prefixes: &["go"],
        category: "Go",
        savings_pct: 85.0,
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?black(\s|$)",
        rtk_cmd: "rtk black",
        rewrite_prefixes: &["python3 -m black", "python -m black", "black"],
        category: "Python",
        savings_pct: 70.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?flake8(\s|$)",
        rtk_cmd: "rtk flake8",
        rewrite_prefixes: &["python3 -m flake8", "python -m flake8", "flake8"],
        category: "Python",
        savings_pct: 75.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?pylint(\s|$)",
        rtk_cmd: "rtk pylint",
        rewrite_prefixes: &["python3 -m pylint", "python -m pylint", "pylint"],
        category: "Python",
        savings_pct: 75.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?pyright(\s|$)",
        rtk_cmd: "rtk pyright",
        rewrite_prefixes: &["python3 -m pyright", "python -m pyright", "pyright"],
        category: "Python",
        savings_pct: 75.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?isort(\s|$)",
        rtk_cmd: "rtk isort",
        rewrite_prefixes: &["python3 -m isort", "python -m isort", "isort"],
        category: "Python",
        savings_pct: 65.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?bandit(\s|$)",
        rtk_cmd: "rtk bandit",
        rewrite_prefixes: &["python3 -m bandit", "python -m bandit", "bandit"],
        category: "Python",
        savings_pct: 70.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?coverage\s+(run|report|html|xml|json|combine|erase)",
        rtk_cmd: "rtk coverage",
        rewrite_prefixes: &["python3 -m coverage", "python -m coverage", "coverage"],
        category: "Python",
        savings_pct: 75.0,
        subcmd_savings: &[("report", 80.0)],
        subcmd_status: &[
            ("run", RtkStatus::Passthrough),
            ("html", RtkStatus::Passthrough),
            ("xml", RtkStatus::Passthrough),
            ("json", RtkStatus::Passthrough),
            ("combine", RtkStatus::Passthrough),
            ("erase", RtkStatus::Passthrough),
        ],
    },
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?tox(\s|$)",
        rtk_cmd: "rtk tox",
        rewrite_prefixes: &["python3 -m tox", "python -m tox", "tox"],
        category: "Python",
        savings_pct: 80.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^(?:python3?\s+-m\s+)?nox(\s|$)",
        rtk_cmd: "rtk nox",
        rewrite_prefixes: &["python3 -m nox", "python -m nox", "nox"],
        category: "Python",
        savings_pct: 80.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^pip-compile(\s|$)",
        rtk_cmd: "rtk pip-compile",
        rewrite_prefixes: &["pip-compile"],
        category: "Python",
        savings_pct: 70.0,
        subcmd_savings: &[],
        subcmd_status: &[],
    ...
    RtkRule {
        pattern: r"^pip-sync(\s|$)",
        rtk_cmd: "rtk pip-sync",
        rewrite_prefixes: &["pip-sync"],
        category: "Python",
        savings_pct: 65.0,
        subcmd_savings: &[],
        subcmd_status: &[],
